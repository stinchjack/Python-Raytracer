...
def view_create(eye_z,  physical_rectangle, view_rectangle, transform=None):
    ...
    view = ['view', lightingmodel_basic_create(), None,
            ...
            cartesian_create((view_rectangle['left'] + view_rectangle['right']) / mpfr(
                2.0), (view_rectangle['top'] + view_rectangle['bottom']) / mpfr(2.0), eye_z),
            {}, None, None]
...
def view_setAntialias(view, on=False, x=1, y=1, stochastic=False):
    ...
    if on:
        ...
        view[VIEW_ANTIALIAS_DATA]['x_step'] = (view[VIEW_VIEWRECTANGLE]['right'] - view[VIEW_VIEWRECTANGLE][
                                               'left']) / (view[VIEW_PHYSICALRECTANGLE]['right'] - view[VIEW_PHYSICALRECTANGLE]['left'])
        view[VIEW_ANTIALIAS_DATA]['y_step'] = (mpfr(view[VIEW_VIEWRECTANGLE]['bottom']) - view[VIEW_VIEWRECTANGLE][
                                               'top']) / (mpfr(view[VIEW_PHYSICALRECTANGLE]['bottom']) - mpfr(view[VIEW_PHYSICALRECTANGLE]['top']))
    ...
    if view[VIEW_ANTIALIAS]['on'] and not view[VIEW_ANTIALIAS]['stochastic']:
        ...
        while a_view_x < view_x:
            ...
            while a_view_y < view_y:
                ...
                if (result != False):
                    result['ray'] = ray
                    colour = colour_scale(view[VIEW_LIGHTINGMODEL][LIGHTINGMODEL_BASIC_CALCFUNC](
                        view[VIEW_LIGHTINGMODEL], scene_obj, result), colour_sc)
        ...
        for i in range(count):
            a_view_x = random.uniform(
                view[VIEW_VIEW_X], view_x + view[VIEW_ANTIALIAS_DATA]['x_step'])
            a_view_y = random.uniform(
                view[VIEW_VIEW_Y], view_y + view[VIEW_ANTIALIAS_DATA]['y_step'])
            ray = ray_create(eye, cartesian_create(
                a_view_x - eye[1], a_view_y - eye[2], zero - eye[3]))
            result = scene_obj.testIntersect(ray)
            if (result != False):
        ...
        ray = ray_create(eye, cartesian_create(
            view[VIEW_VIEW_X] - eye[1], view[VIEW_VIEW_Y] - eye[2], zero - eye[3]))
...
def view_render(view, scene_obj, output_type):
    ...
    x_step = (view[VIEW_VIEWRECTANGLE]['right'] - view[VIEW_VIEWRECTANGLE]['left']) / \
        (view[VIEW_PHYSICALRECTANGLE]['right'] -
         view[VIEW_PHYSICALRECTANGLE]['left'])
    y_step = (mpfr(view[VIEW_VIEWRECTANGLE]['bottom']) - view[VIEW_VIEWRECTANGLE]['top']) / (
        mpfr(view[VIEW_PHYSICALRECTANGLE]['bottom']) - mpfr(view[VIEW_PHYSICALRECTANGLE]['top']))
