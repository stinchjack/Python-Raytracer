...
def shape_capped_cylinder_diffuse_colour(shape, intersect_result):
	...
	if (intersect_result['shape_part'] == 'topcap_result' and
		shape[SHAPE_DATA]['topcap'] != None and
		...
		shape[SHAPE_DATA]['bottomcap'] != None and
...
def shape_cone_create(colour, specular, y_top=None, y_bottom=None,\
	...
	y_top: The minimum Y value in an untransformed cone. If this parameter is
		None, the default of 0 will be used.
...
def shape_capped_cone_diffuse_colour(shape, intersect_result):
	...
	if intersect_result['shape_part'] == 'topcap_result':
		if shape[SHAPE_DATA]['topcap'] != None:
...
def shape_capped_cone_create (colour, specular, topcap={}, \
	bottomcap={}, y_top=None, y_bottom = None, transform = None):
	...
	Returns: a tuple with the data needed to render a capped cone.
	"""	
	shape = shape_cone_create(colour, specular, y_top, y_bottom, transform)
	if type(topcap) is dict: shape[SHAPE_DATA]['topcap'] = topcap
	if type(bottomcap) is dict: shape[SHAPE_DATA]['bottomcap'] = bottomcap
